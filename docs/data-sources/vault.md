...
Only the decrypted `yaml` is stored in Terraform state. The `vault_file` and password attributes are read from configuration on every plan/apply and are never persisted to state. For workflows where even the decrypted content must not appear in state, use the [`ansible_vault` ephemeral resource](../ephemeral-resources/vault.md) instead (requires Terraform 1.10+).
